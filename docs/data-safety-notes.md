...
Yes, through the public support and data-deletion pages.

The app now exposes a public deletion request path intended for Play Console and similar store requirements:

- `/data-deletion`
- `/delete-account` as a route alias

The current policy is email-based account deletion through `support@notebill.app`.

The app also supports deleting saved invoices directly, but that is separate from an account-level deletion request.
...
5. Confirm the public deletion page is reachable on the production domain and the support inbox is monitored
